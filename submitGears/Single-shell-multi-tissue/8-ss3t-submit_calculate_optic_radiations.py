...
analysis_label = 'SS3T - traceOpticRadiation_%s_%s' % (qp.gear.version, now)
...
subject_fod = [ana for ana in analyses if ana.label.startswith('SS3T - createSubjectFODMap')]
segment_thalamic = [ana for ana in analyses if ana.label.startswith('segmentThalamicNuclei')]

calculate_fixels = [ana for ana in analyses if ana.label.startswith('SS3T - calculateFixels')]
template_fod = [ana for ana in analyses if ana.label.startswith('SS3T - createGroupFODTemplate')]
...
for sub in subjects:
    ...
    if subject_label == 'TOME_3004':
        ...
        config = {'minFODampLeft':'0.15', 'minFODampRight':'0.2'}
    elif subject_label == 'TOME_3020':
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.1'}
    elif subject_label == 'TOME_3023':
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.15'}
    elif subject_label == 'TOME_3026':
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.15'}
    elif subject_label == 'TOME_3028':
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.15'}
    elif subject_label == 'TOME_3029':
        config = {'minFODampLeft':'0.18', 'minFODampRight':'0.2'}
    elif subject_label == 'TOME_3030':
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.15'}
    elif subject_label == 'TOME_3032':
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.15'}
    elif subject_label == 'TOME_3033':
        config = {'minFODampLeft':'0.1', 'minFODampRight':'0.1'}
    elif subject_label == 'TOME_3036':
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.15'}
    else:
        config = {'minFODampLeft':'0.2', 'minFODampRight':'0.2'}
    if subject_label in subject_list:
        ...
        try:
            _id = qp.run(analysis_label=analysis_label,
                          inputs=inputs, config=config, destination=fw.get(session_id))
