...
for i in save_destination.analyses:
    if i.label.startswith('SS3T - createGroupFODTemplate'):
        fod_files.append(i)       
...
for i in save_destination.analyses:
    if i.label.startswith('SS3T - register-thalamic-segmentations'):
...
analysis_label = 'SS3T - left - track-single-fod-tract_%s_%s' % (qp.gear.version, now)
...
# Set configs for left track and submit
analysis_label = 'SS3T - right - track-single-fod-tract_%s_%s' % (qp.gear.version, now)
config = {'dilateSeedMask': '1',
