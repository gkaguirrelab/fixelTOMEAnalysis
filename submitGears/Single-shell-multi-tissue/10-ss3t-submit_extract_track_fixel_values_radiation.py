...
analysis_label = 'SS3T - extractTrackFixelValues_%s_%s' % (qp.gear.version, now)
...
for i in save_destination.analyses:
    if i.label.startswith('SS3T - calculateFixels'):
...
left_track_values = []
for i in save_destination.analyses:
    if i.label.startswith('SS3T - left - calculateMaskIntersection'):
        left_track_values.append(i)       
run_to_use = left_track_values[-1]
for file in run_to_use.files:
    if 'left_mask' in file.name:
        left_track = file         
        
# Get the trackSingleFodTrack right gear output
right_track_values = []
for i in save_destination.analyses:
    if i.label.startswith('SS3T - right - calculateMaskIntersection'):
        right_track_values.append(i)       
run_to_use = right_track_values[-1]
for file in run_to_use.files:
    if 'right_mask' in file.name:
        right_track = file                 
...
inputs = {'calculateFixelResults': calculate_fixel_results, 
          'leftROI': left_track}        
analysis_label = 'SS3T - left - extractTrackFixelValues_%s_%s' % (qp.gear.version, now)
...
analysis_label = 'SS3T - right - extractTrackFixelValues_%s_%s' % (qp.gear.version, now)
