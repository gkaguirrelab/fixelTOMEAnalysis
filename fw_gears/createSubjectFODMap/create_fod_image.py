...
def create_fod_image(preprocessed_dti, mask, bvecs, bvals, response_wm, response_gm, response_csf, lmax, mrtrix_bin_path, subject_id, workdir, output_dir, use_highest_b=False, n_threads='0'):
    ...
    if use_highest_b == False:
        os.system('%s %s -fslgrad %s %s %s' % (os.path.join(mrtrix_bin_path, 'mrconvert'), preprocessed_dti, bvecs, bvals, preprocessed_dti_mif))  
    else:
        embedded_image = os.path.join(workdir, 'b_embedded.mif')
        os.system('%s %s -fslgrad %s %s %s' % (os.path.join(mrtrix_bin_path, 'mrconvert'), preprocessed_dti, bvecs, bvals, embedded_image))  
        os.system('%s -shells 0,%s %s %s' % (os.path.join(mrtrix_bin_path, 'dwiextract'), str(maximum_b), embedded_image, preprocessed_dti_mif))
    ...
    os.system('%s %s regrid -vox 1.25 %s -nthreads %s' % (os.path.join(mrtrix_bin_path, 'mrgrid'),
                                                          preprocessed_dti_mif, upsampled_dti, n_threads))
    os.system('%s %s regrid -vox 1.25 %s -nthreads %s' % (os.path.join(mrtrix_bin_path, 'mrgrid'),
                                                          mask, upsampled_mask, n_threads))
    ...
    if use_highest_b == False:
        ...
        if lmax == 'NA':
            os.system('%s msmt_csd %s %s %s %s %s %s %s -mask %s -nthreads %s' % (os.path.join(mrtrix_bin_path, 'dwi2fod'), upsampled_dti,
                                                                                  response_wm, wm_fod, response_gm, gm_fod, response_csf, csf_fod,
                                                                                  upsampled_mask, n_threads))
        else:
            os.system('%s msmt_csd %s %s %s %s %s %s %s -mask %s -lmax %s -nthreads %s' % (os.path.join(mrtrix_bin_path, 'dwi2fod'), upsampled_dti,
                                                                                           response_wm, wm_fod, response_gm, gm_fod, response_csf, csf_fod,
                                                                                           upsampled_mask, lmax, n_threads))
    else:
        os.system('%s -version' % os.path.join(mrtrix_bin_path, 'ss3t_csd_beta1'))
        os.system('%s -mask %s -nthreads %s %s %s %s %s %s %s %s' % (os.path.join(mrtrix_bin_path, 'ss3t_csd_beta1'), upsampled_mask, n_threads, upsampled_dti,
                                                                     response_wm, wm_fod, response_gm, gm_fod, response_csf, csf_fod))        
