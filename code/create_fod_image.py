...
def create_fod_image(preprocessed_dti, mask, bvecs, bvals, response_wm, response_gm, response_csf, mrtrix_bin_path, fsl_bin_path, subject_id, workdir, output_dir):
    ...
    if response_wm == '':
        ...
        os.system('%s dhollander %s -fslgrad %s %s %s %s %s -force -quiet' % (os.path.join(mrtrix_bin_path, 'dwi2response'),
    ...
    os.system('%s %s regrid -vox 1.25 %s -force -quiet' % (os.path.join(mrtrix_bin_path, 'mrgrid'),
    ...
    os.system('%s msmt_csd %s %s %s %s %s %s %s -mask %s -lmax 8,8,8 -force -fslgrad %s %s -quiet' % (os.path.join(mrtrix_bin_path, 'dwi2fod'), upsampled_dti,
    ...
    os.system('%s %s %s -fslgrad %s %s -force -quiet' % (os.path.join(mrtrix_bin_path, 'dwi2mask'), upsampled_dti, normalize_mask, bvecs, bvals))
    ...
    os.system('%s %s %s %s %s %s %s -mask %s -force -quiet' % (os.path.join(mrtrix_bin_path, 'mtnormalise'), wm_fod, normalized_wm_fod,
